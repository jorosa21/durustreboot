                ...
                this.headerForm.setValue({
                  ...
                  this.holidayManagementService.detailList(this.holidayID).subscribe(data => { 
                    for (var i = 0; i < data.length; i++) {
                      var selectedHolidayType = this.holidayType.filter(x => x.id ===  data[i].holiday_type_id)[0]
                      this.holidayList.push({
                        holiday_name: data[i].holiday_name,
                        holiday_type_id: data[i].holiday_type_id,
                        holidayTypeDesc: selectedHolidayType['description'],
                        holiday_date: this.pipe.transform(data[i].holiday_date, 'MM/dd/yyyy'),
                      })
                    }
